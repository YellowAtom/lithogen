...
#ifdef OS_WINDOWS
	// Give code its own scope so the variables get garbage collected in program lifetime.
	{
		// Set the windows title bar to dark as the application is always dark.
		constexpr BOOL darkMode = true;
		DwmSetWindowAttribute(glfwGetWin32Window(mainWindow), DWMWA_USE_IMMERSIVE_DARK_MODE, &darkMode, sizeof(darkMode));
	}
#endif
